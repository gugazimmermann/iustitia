...
function attchmentName(tenant: string, ownertId: string, originalname: string): string {
  ...
  return `${tenant}/${ModulesEnum.attachments}/${ownertId.split("-").join("")}/${originalname}_${now}`
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function sendAttchment(file: any, fileName: string): Promise<AWS.S3.ManagedUpload.SendData> {
  const params = {
    Bucket: process.env.NX_BUCKET_FILES as string,
    Key: fileName,
    Body: file.buffer
  };
...
export async function deleteOne(req, res) {
  ...
  try {
    ...
    await deleteFromBucket(data.link, process.env.NX_BUCKET_FILES as string)
